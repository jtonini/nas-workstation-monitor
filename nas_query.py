...
def show_failures() -> None:
    ...
    if not recent:
        ...
        for r in recent:
            # Extract values from tuple (in order returned by SQL)
            ws = r[0]              # workstation
            mount = r[1]           # mount_point
            failed = r[2]          # first_failure (failed_at)
            last_failure = r[3]    # last_failure
            failure_count = r[4]   # failure_count
            resolved = r[5]        # resolved (0 or 1)
            resolved_at = r[6]     # resolved_at (timestamp or None)
...
if __name__ == '__main__':
    ...
    # failures command
    subparsers.add_parser('failures', help='Show unresolved mount failures and recent history')
